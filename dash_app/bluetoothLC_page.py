...
def fetchSLC(timeDelta, wireless, btaddress ):
        try:
                #print("trying database")
                con = mdb.connect('localhost', 'root', config.MySQL_Password, 'SmartGarden3');
                cur = con.cursor()
...
def buildSoilTemperature_Conductivity_Graph_Figure(wireless, btaddress):
    
    timeDelta = datetime.timedelta(days=NUMBEROFDAYS)
    records = fetchSLC(timeDelta, wireless, btaddress)
    ...
    for record in records:
        ...
        Light.append(record[0])
        Conductivity.append(record[1])
    ...
    if (len(records) == 0):
        ...
        fig.update_layout(
            height=100,
            title_text='No Soil Data Available')
        return fig
    ...
    # Add traces
    fig.add_trace(
        go.Scatter(x=Time, y=Light, name="Soil Light ",
        line = dict(
                    color = ('red'),
                    width = 2,
                    ),
    ...
    fig.add_trace(
        go.Scatter(x=Time, y=Conductivity, name="Soil Conductivity", 
        line = dict(
                    color = ('blue'),
                    width = 2,
                    ),
    ...
    # Add figure title
    fig.update_layout(
        title_text="Soil Light and Conductivity ", height=400
    )

    # Set x-axis title
    fig.update_xaxes(title_text="Time")
   
    minTemp = min(Light)*0.9
    maxTemp = max(Light)*1.10
    minCond = min(Conductivity)*0.9
    maxCond = max(Conductivity)*1.10
    # Set y-axes titles
    fig.update_yaxes(title_text="<b>Light (lux)</b>", range = (minTemp, maxTemp), secondary_y=False, side='left')
    fig.update_yaxes(title_text="<b>Conductivity(µS/cm)</b>", range = (minCond,maxCond), secondary_y=True, side='right')
...
def buildSoilTemperature_Humidity_Graph(wireless, bluetooth):
    ...
    graph =  dcc.Graph(
                    id = {'type' : 'BTGdynamic', 'index': 'graph-oth' },
                    figure=fig,
                    animate = False
                    )
...
def buildSoilBrightness_Conductivity_Graph(wireless, bluetooth):

    fig = buildSoilTemperature_Conductivity_Graph_Figure(wireless, bluetooth)

    graph = dcc.Graph(
                    id = {'type' : 'BTGdynamic', 'index': 'graph-olc' },
                    figure=fig,
                    animate = False
                    )
    return graph
...
def build_th_graphs(WirelessList):
    ...
    for wireless in WirelessList:  
       ...
       if (len(bluetoothaddresses) == 0):
        ...
        output.append( html.H2(wireless["name"] + " / " +wireless["id"] + " Soil Light / Conductivity "))
...
def BluetoothLCPage():
    ...
    layout = html.Div(children=[
        ...
        html.Div(id={'type' : 'BluetoothGraphs', 'index' : 0}, children = build_th_graphs(WirelessList)),
